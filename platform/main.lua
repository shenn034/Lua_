...
function love.update(dt)
  ...
  if #coins == 0 and gameState == 2 then
    ...
    if timer < saveData.bestTime then
      ...
      love.filesystem.write("data.lua", table.show(saveData, "saveData"), size)
    end
  end
end
...
function love.draw()
  ...
  love.graphics.draw(player.sprite, player.body:getX(), player.body:getY(), r, player.direction, 1, sprites.player_stand:getWidth()/2, sprites.player_stand:getHeight()/2, kx, ky)
...
function spawnPlatform(x, y, width, height)
  ...
  platform.fixture = love.physics.newFixture(platform.body, platform.shape, density)
